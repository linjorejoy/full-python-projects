...
def iterate_through_each_json_file_in_folder(path):
    ...
    for filename in os.listdir(path=path):
        if filename.endswith(".json"):
            ...
            this_folder_all_json_obj[file_absolute_name] = get_json_data(file_absolute_name)
...
if __name__ == "__main__":
    ...
    svc.add_image(iterate_through_each_json_file_in_folder(dir_name))

    with open("result.json", mode="w") as final_result:
        json.dump(obj=all_json_obj, fp=final_result, indent=4)
