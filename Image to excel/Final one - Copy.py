...
for i in range (0,colourPixels.size[0]):
    for j in range (0,colourPixels.size[1]):
        worksheet.write(3*i + 0 , j , df["red"][   i*( colourPixels.size[1])+j ])
        worksheet.write(3*i + 1 , j , df["green"][ i*( colourPixels.size[1])+j ])
        worksheet.write(3*i + 2 , j , df["blue"][  i*( colourPixels.size[1])+j ])
